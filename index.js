  ...
  try {
    ...
    app.post("/depositWithdraw", async (req, res) => {
      ...
      if(applicant.type === 'deposit'){
         updateDoc = {
          $set: {
            availableAmount: parseFloat( previousAmount.availableAmount) + parseFloat(applicant.deposit)
          },
        };
      }
      else{
         updateDoc = {
          $set: {
            availableAmount: parseFloat( previousAmount.availableAmount) - parseFloat(applicant.withdraw)
          },
        };
      }
    ...
    app.post("/sentMoney", async (req, res) => {
      ...
      const previousAmount = await allAccountsCollection.findOne(filterReceiver);

      if(previousAmount){
        const filter = {accountId:info.id,approve: true};
        const updateDoc = {
          $set: {
            availableAmount: parseFloat( previousAmount.availableAmount) + parseFloat(info.amount)
          },
        };
        const increaseReceiverMoney = await allAccountsCollection.updateOne(filter,updateDoc);

        const filter1 = {email:info.senderEmail};
        const previousAmount1 = await allAccountsCollection.findOne(filter1);
        const updateDoc1 = {
          $set: {
            availableAmount: parseFloat( previousAmount1.availableAmount) - parseFloat(info.amount)
          },
        };
        const decreaseReceiverMoney = await allAccountsCollection.updateOne(filter1,updateDoc1);
